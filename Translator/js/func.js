...
function init(){  
   ...
   // document.getElementById("memo").value = abc;
   showAllData();  
}  
window.onload = init;
// click事件
...
function addData(name,message,time){  
   db.transaction(function(tx){  
       tx.executeSql("INSERT INTO MyData2 VALUES(?,?,?)",
           [name,message,time],
...
function showAllData(){  
   db.transaction(function(tx){  
       ...
       tx.executeSql("CREATE TABLE IF NOT EXISTS MyData2(name TEXT,message TEXT,time INTEGER)");  
       tx.executeSql("SELECT * FROM MyData2",[],function(tx,rs){  
           removeAllData();  
...
function deleteData(){  
  ...
  //  var delid = document.getElementById("did").value; 
    db.transaction(function(tx){  
        tx.executeSql("delete from MyData2 where name =?",[name],
                   null,null); }  
        );
...
function del(){
    db.transaction(function(tx) {
        tx.executeSql("DROP TABLE MyData2");
        })
    showAllData();
}
...
function search(){  
    db.transaction(function(tx){  
        ...
        var name = document.getElementById("name").value; 
        tx.executeSql("CREATE TABLE IF NOT EXISTS MyData2(name TEXT,message TEXT,time INTEGER)");  
        tx.executeSql("SELECT * FROM MyData2 WHERE name = ?",[name],function(tx,rs){  
            removeAllData();  
            for (var i = 0; i < rs.rows.length; i++){  
                showData(rs.rows.item(i));  
            }  
